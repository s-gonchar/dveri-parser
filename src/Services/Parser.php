...
class Parser
{
    private const URI_SPB = 'https://dveri.com/export/json/spb';
    private const DOMAIN = 'dveri.com';

    private Client $client;
    private LogRepository $logRepository;
    private ColorRepository $magazineRepository;
    private LogService $logService;
    private CategoryNestedTreeRepository $themeRepository;

    public function __construct(
        LogService $logService,
        LogRepository $logRepository,
        CategoryNestedTreeRepository $themeRepository,
        ColorRepository $magazineRepository
    ) {
    ...
    public function parseThemes()
    {
        ...
        foreach ($themes as $theme) {
            $theme = $this->themeRepository->findOneByExternalId($theme->id)
                ?: Category::create($theme->title, $theme->id);
     ...
     * @throws \Exception
     */
    private function parseMagazinesByTheme(Category $theme)
    {
...
//                );
//                $responseData = json_decode($response->getBody(), true);
                $magazine = Color::create($magazineData);
